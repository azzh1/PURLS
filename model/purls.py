...
class Purls(Base):
    def __init__(self, num_epoch, input_size=256, 
                 ...
                 n_head=2, k_dim=150, local_type = 'full', use_d = False):
        """
    ...
    def loss(self, skel_emb, desc_emb, class_emb, curr_epoch, target,
             target2=None, is_train=False, is_test=False, **params):
        """
        Argument explanations:
        skel_emb: Pre-trained skeleton features of the batch examples from backbones
        txt_emb: Pre-trained description encoding features of the batch examples from CLIP
        class_emb: Pre-trained description encoding features of candidate classes (seen/unseen) from CLIP
        ...
        """
        # prepare contrastive learning labels
        batch_labels = len(skel_emb) * get_rank() + torch.arange(len(skel_emb), device= skel_emb.device)
        ...
        # calculate loss
        loss = 0.
        bs, p, d = total_skel_emb.shape # shape: batchsize, # of representations, representation feature dim
        total_skel_emb = total_skel_emb.view(bs, -1).to(total_skel_emb.device)
        proj_emb = self.skel_encoder(total_skel_emb) # skel -> txt projection
        proj_emb = proj_emb.view(bs, p, -1).to(total_skel_emb.device) # bs, p, 512

        # each representation contrastive learning loss (include global)
        for i in range(proj_emb.shape[1]):
            curr_proj_emb = proj_emb[:, i, :]
            curr_desc_emb = local_desc_emb[:, i, :]
            curr_loss, _ =  self.loss_func(curr_proj_emb, curr_desc_emb, None, 
                                            self.tmps[i], batch_labels) #, reduction='none')
            # if i == proj_emb.shape[1] - 1:
            #     glob_loss = curr_loss
            if self.use_d:
        ...
        #     loss = global_loss
        global_emb = proj_emb[:, -1, :]
        acc, preds = accuracy(global_emb, target, class_emb[:, -1, :], is_test)
        # else:
